...
- `components/` — grouped by feature: `landing/`, `auth/`, `courses/`, `lesson/`, `quiz/`, `certificates/`, `dashboard/teacher/`, `dashboard/student/`, `layout/`, `theme/` (`ThemeToggle`, `ThemeScript`, `LanguageSwitcher`), `analytics/` (`AnalyticsInit`), `ui/` (generic primitives: `Button`, `Card`, `Badge`, `ProgressBar`, `Spinner`, `Container`, `FormField`).
...
Light/dark theme is a `data-theme` attribute on `<html>`, toggled by `ThemeProvider`/`ThemeToggle` and persisted to `localStorage`; `components/theme/ThemeScript.tsx` is inlined in `<head>` to apply the saved theme before first paint and avoid a flash — keep it a plain inline script, not a component that renders after hydration.
