...
Visual design: colors/fonts are plain Tailwind arbitrary-value classes (`bg-[#0B1224]`, `text-[#D4A94F]`, etc.) plus CSS variables in `app/globals.css`; there's no `tailwind.config` theme extension (Tailwind v4 CSS-first config). Fonts (Amiri for display/Arabic headings, Tajawal for body) are loaded via `next/font/google` in `app/[locale]/layout.tsx` — don't reintroduce a manual `@import` for them in CSS (breaks `@import`-must-be-first CSS ordering with Tailwind's own `@import "tailwindcss"`). Light/dark theme is a `data-theme` attribute on `<html>`, toggled by `ThemeProvider`/`ThemeToggle` and persisted to `localStorage`; `components/theme/ThemeScript.tsx` is inlined in `<head>` to apply the saved theme before first paint and avoid a flash — keep it a plain inline script, not a component that renders after hydration.
