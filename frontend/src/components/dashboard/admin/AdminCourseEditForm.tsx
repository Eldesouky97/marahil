...
export function AdminCourseEditForm({ course, onSaved }: { course: Course; onSaved: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setSaving(true);
    await updateCourse(course.id, { title, description, stage, subject, coverImageUrl });
    setSaving(false);
    onSaved();
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 rounded-xl border border-border bg-surface-2 p-5">
      ...
      <FormField label={tCourseForm("coverImage")}>
        <ImageUploadField folder="course-covers" currentUrl={coverImageUrl} onUploaded={setCoverImageUrl} />
      </FormField>
